...
| 지표 | 값 | 해석 |
| --- | --- | --- |
| Numeric Hallucination Rate | **19.3% → 0.0%** | 게이트를 통과한 환각 숫자 0건 |
| Auto-Discard Recall | **100.0%** | 라벨된 환각 42건 전부 폐기(FN=0) |
| Auto-Discard Precision | **59.2%** | 정상 필드 29건이 함께 폐기됨 |
| Grounded-Accuracy | **86.7%** | 전체 218필드 중 최종적으로 올바르게 verified 된 비율 |
...
재현율 100%를 유지한 채 정밀도 35.8% → **59.2%**, 근거정확도 64.4% → **86.7%**
(벤치 규모가 1/3 늘어난 상태에서). 남은 것은 진짜 모호성입니다. 두 필드가 실패를
똑같이 잘 설명하면 둘 다 폐기됩니다.

### NumHall-KR이 무엇이고, 무엇이 아닌가

- **맞는 것**: 결정적·재현 가능한 **회귀 스위트**. 58문서/218라벨필드 =
  **합성 50**(세금계산서 30 + 시산표 10 + 재무상태표 10, 고정 시드 생성기) +
  **수동 작성 8**. 동봉된 룰팩은 전부 벤치에서 돌아갑니다 — 벤치가 한 번도 돌리지 않는
...
- **모호할 때는 여전히 과폐기** — fault localization이 위반을 설명하는 최소 조합까지
  ...
  경우) 둘 다 폐기됩니다(정밀도 59.2%). 룰팩에 서로 겹치는 불변식이 많을수록 좁히기가
...
- ~~**Fault localization**~~ — 반영 완료. 정밀도 35.8% → 59.2%, 근거정확도 64.4% → 86.7%,
